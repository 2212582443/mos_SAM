...
class Sam(nn.Module):
    ...
    def forward(
        self,
        image: Tensor = None,  # (bs, 3, h, w)
        point_coords: Tensor = None,  # (bs, N, 2)
        point_labels: Tensor = None,  # (bs, N)
        boxes: Tensor = None,  # (bs, 4)
        mask_inputs: Tensor = None,  # (bs, 1, h, w)
        ...
        if True:
            ...
            masks = (low_res_masks > self.mask_threshold).float()

        # low_res_masks = normalize(threshold(low_res_masks, 0.0, 0))
        # low_res_masks = (10 * low_res_masks).sigmoid()
        # masks: (bs, N, h, w)
        # iou_predictions: (bs, N)
        # low_res_masks: (bs, N, 256, 256)
