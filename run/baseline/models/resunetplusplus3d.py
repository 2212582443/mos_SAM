...
class ASPP(nn.Module):
    def __init__(self, in_dims, out_dims, rate=[6, 12, 18]):
        ...
        self.aspp_block1 = nn.Sequential(
            nn.Conv3d(in_dims, out_dims, 3, stride=1, padding=rate[0], dilation=rate[0]),
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_dims),
        )
        self.aspp_block2 = nn.Sequential(
            nn.Conv3d(in_dims, out_dims, 3, stride=1, padding=rate[1], dilation=rate[1]),
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_dims),
        )
        self.aspp_block3 = nn.Sequential(
            nn.Conv3d(in_dims, out_dims, 3, stride=1, padding=rate[2], dilation=rate[2]),
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_dims),
        )
...
class ResUnetPlusPlus3d(nn.Module):
    def __init__(self, in_channels=1, out_channels=5, filters=[32, 64, 128, 256, 512]):
        super().__init__()
        ...
        self.input_skip = nn.Sequential(nn.Conv3d(in_channels, filters[0], kernel_size=3, padding=1))
        ...
        self.output_layer = nn.Conv3d(filters[0], out_channels, 1)
